  ...
  eksekusiMenuSpesifik: async (suratPerintah, currentSessionRole, contextUser = {}) => {
    ...
    try {
      ...
      if (aksi === 'ambil_data') {
        ...
        const offset = Math.min(Math.max(Number(suratPerintah.offset) || 0, 0), 1000000);
        const result = await db.query(`SELECT ${requested.map(quoteIdentifier).join(', ')} FROM ${quoteIdentifier(table)} ORDER BY ${quoteIdentifier(orderColumn)} ASC LIMIT $1 OFFSET $2`, [limit, offset]);
      ...
      if (aksi === 'tambah_data') {
        ...
        const result = await db.query(`INSERT INTO ${quoteIdentifier(table)} (${keys.map(quoteIdentifier).join(', ')}) VALUES (${keys.map((_, i) => `$${i + 1}`).join(', ')}) RETURNING ${safeColumns.map(quoteIdentifier).join(', ')}`, keys.map((key) => input[key]));
        await audit(contextUser, aksi, table, 'SUCCESS');
