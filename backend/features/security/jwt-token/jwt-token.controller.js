...
class JwtTokenController {
  async issueSessionToken(userData, options = {}) {
    if (options === true && userData?.refreshToken) return this.rotateSessionToken(String(userData.refreshToken));
    try {
  ...
  async validateActiveToken(token) {
    ...
    if (!token || token.length < 40) return access;
    const result = await db.query(
      `SELECT s.id, s.user_id, l.username, l.role FROM auth_sessions s JOIN login l ON l.id = s.user_id
       WHERE s.refresh_token_hash = $1 AND s.revoked_at IS NULL AND s.expires_at > NOW() LIMIT 1`,
      [sessionRepository.hashRefreshToken(token)]
    );
    if (!result.rows[0]) return { success: false, expired: true, error: 'Sesi refresh tidak aktif.' };
    const row = result.rows[0];
    return { success: true, data: { id: Number(row.user_id), username: String(row.username), role: normalizeRole(row.role), refreshToken: token }, user: { id: Number(row.user_id), username: String(row.username), role: normalizeRole(row.role), sessionId: String(row.id) } };
  }
  ...
  expressAuthenticateToken = async (req, res, next) => {
    try {
      ...
      const validation = await jwtTokenService.verifyAccessToken(token);
      ...
      return next();
    } catch (error) { return res.status(500).json({ success: false, error: 'Gagal memvalidasi sesi.' }); }
  };
